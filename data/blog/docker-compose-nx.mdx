...
date: "2023-08-28"
summary: "Turbocharge Your Full-Stack Projects: Dockerizing Next.js, Nest.js, Redis and MongoDB with Nx Monorepo and Docker Compose"
tags: ["Docker", "Nx monorepo", "NextJs", "NestJs", "redis", "MongoDb"]
draft: true
---

Discover how to leverage Docker to streamline the development workflow of your applications. Suppose you have an application that has 2 frontends and one backend. It uses a Redis database and a MongoDB database. You want to Dockerize it for ease of development and deployment. In that case, this blog post is for you.
...
3. Install and then create a NextJS **frontend**, **admin** application
    ...
    > Go through the prompts and select appropriate configuration, we are going for minimal with **CSS**, **no E2E test** and an **APP Router**.

4. Add Redis and MongoDB databases to the backend application
...
5. Implement Redis and MongoDB in the backend application
...
6. Initialize git repository and make the initial commit
...
2. Define services for each of your projects: the two frontends, the backend, Redis, and MongoDB.
...
6. Set up bind mounts for each service to enable live reloading. This mounts will persist the changes made in the host machine `node_modules` & the code to the container.
